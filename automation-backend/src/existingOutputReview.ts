...
export interface AurumRouteResolution {
  ...
  // Same resolution, keyed by route type (landing/webCompleta/...) instead
  // of canonical path — convenient for callers that need to reason about
  // "this type's existing component" (e.g. premium-component preservation).
  componentByType: Record<string, string>;
  reusedTypes: string[];
...
export function resolveAurumRouteComponents(appContent: string, slug: string): AurumRouteResolution {
  ...
  for (const type of Object.keys(canonicalRouteByType)) {
    ...
    if (resolved) {
      ...
      componentByType[type] = resolved;
      reusedTypes.push(type);
    }
  }

  return { componentByCanonicalRoute, componentByType, reusedTypes, ambiguousTypes };
}

// Pulls the data-file import out of an existing AURUM component. Handles
// both the alias form (`@/data/clientDemos/sandhouse`) and relative forms
// (`./data/clientDemos/sandhouse`, `../data/clientDemos/sandhouse`) — real
// hand-written components don't always use the `@/` alias — and whatever
// export name the file actually uses (e.g. `sandhouseDemo`, not just a name
// that happens to match the slug). Returns null when the component doesn't
// import from clientDemos at all.
...
// ─── Premium component classification ──────────────────────────────────────
//
// "Reusing a component's name does not mean overwriting its content." Before
// regenerating any AURUM route component we must know whether the existing
// file is a hand-authored premium build (motion, custom cursor, editorial
// copy, ...) or something the orchestrator itself produced and can safely
// regenerate. Defaults are conservative: anything that isn't recognizably
// our own auto-generated output is treated as off-limits.

export type AurumComponentClassification = "premium_manual" | "autogenerated_safe_to_replace" | "unknown";

const AUTOGENERATED_MARKER = "Auto-generated by immersphere-production-orchestrator";

const PREMIUM_COMPONENT_SIGNALS: RegExp[] = [
  /\bHelmet\b/,
  /react-helmet-async/,
...
export function classifyExistingAurumComponent(content: string): AurumComponentClassification {
  if (!content || !content.trim()) return "unknown";
  if (PREMIUM_COMPONENT_SIGNALS.some((re) => re.test(content))) return "premium_manual";
  if (content.includes(AUTOGENERATED_MARKER)) return "autogenerated_safe_to_replace";
  return "unknown";
}
