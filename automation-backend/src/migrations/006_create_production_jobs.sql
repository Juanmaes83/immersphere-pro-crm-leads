...
-- pipeline jobs (queued -> generating -> pr_created -> merged -> live).
-- UUID primary key, idempotency_key UNIQUE constraint prevents duplicate
-- jobs for the same logical operation.
...
CREATE INDEX IF NOT EXISTS idx_production_jobs_idempotency_key
